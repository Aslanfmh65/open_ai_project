...
class Agent:
    ...
    def __init__(self, actions=4):
        ...
        self.action_in = tf.placeholder(tf.float32, [1], name='action_in')

        self.W = tf.get_variable('W', [self.num_actions])
        self.best_action = tf.argmax(self.W, axis=0)  # return the index of largest value across axis, self.W is input

        action_weight = tf.slice(self.W, self.action_in, [1])
        policy_loss = -(tf.log(action_weight) * self.reward_in)
        self.optimizer = tf.train.AdadeltaOptimizer(learning_rate=1e-3).minimize(policy_loss)
    ...
    def train(self, sess, action, reward):
        sess.run(self.optimizer, {self.action_in: [action],
